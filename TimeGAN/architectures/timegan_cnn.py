...
class GeneratorNetwork(torch.nn.Module):
    ...
    def __init__(self, Z_dim, hidden_dim, num_layers, padding_value, max_seq_len):
        ...
        # )
        self.gen_conv = nn.Sequential(
            nn.Conv2d(in_channels=1, out_channels=20, kernel_size=(3, 3), stride=2, bias=False)
            , nn.BatchNorm2d(20)
            , nn.ReLU()
            , nn.Conv2d(in_channels=20, out_channels=40, kernel_size=(3, 3), stride=2, bias=False)
            , nn.BatchNorm2d(40)
            , nn.ReLU()
            , nn.Conv2d(in_channels=40, out_channels=80, kernel_size=(3, 3), stride=2, bias=False)
            , nn.BatchNorm2d(80)
            , nn.ReLU()
            , nn.Conv2d(in_channels=80, out_channels=100, kernel_size=(3, 3), stride=2, bias=False)
            , nn.BatchNorm2d(100)
            , nn.ReLU()
            , nn.Flatten(start_dim=1)
...
class TimeGAN(torch.nn.Module):
    ...
    def _generator_forward(self, X, T, Z, gamma=1):
        ...
        smooth_labels_L = torch.ones_like(
            Y_fake)  # * 0.9  # torch.tensor(np.random.uniform(0.7, 0.9, Y_fake.size()))  #
