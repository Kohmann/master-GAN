...
class InstanceNoise:
    def __init__(self, end_epoch, noise_level=1.0):
        self.noise_level = noise_level
        self.end_epoch = int(end_epoch)
        self.noise_decay = self.noise_level / self.end_epoch

    def decay(self):
        self.noise_level -= self.noise_decay

    def __mul__(self, other):
        return other * self.noise_level
...
class RTSGAN(torch.nn.Module):
    ...
    def _discriminator_forward(self, X, Z, gamma=10):
        ...
        H_fake = H_fake + torch.rand_like(H_fake, device=self.device) * self.noise_percent
        H_real = H_real + torch.rand_like(H_real, device=self.device) * self.noise_percent
        # Discriminator Loss
